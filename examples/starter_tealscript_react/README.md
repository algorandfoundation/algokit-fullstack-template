...
4. In the case of a smart contract project, execute `algokit generate env-file -a target_network localnet` from the `starter_tealscript_react-contracts` directory to create a `.env.localnet` file with default configuration for `localnet`.
...
The frontend starter also provides an example of interactions with your CalculatorClient in [`AppCalls.tsx`](projects/starter_tealscript_react-frontend/src/components/AppCalls.tsx) component by default.
